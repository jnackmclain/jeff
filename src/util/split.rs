...
fn create_gap_splits(obj: &mut ObjInfo) -> Result<()> {
    ...
    for (addr, split) in new_splits {
        let split_unit = split.unit.clone();
...
fn split_pdata(obj: &mut ObjInfo) -> Result<()> {
    ...
    for (_section_index, section) in obj.sections.by_kind(ObjSectionKind::Code) {
        for (start_addr, split) in section.splits.iter() {
            ...
            // those are our pdata bounds
            let mut split_start: Option<u32> = None;
            let mut split_end: Option<u32> = None;
            for (i, chunks) in pdata_sec.data.chunks_exact(8).enumerate() {
                let target_addr = u32::from_be_bytes(chunks[0..4].try_into()?);
                if target_addr >= start_addr && split_start.is_none() {
                    split_start = Some(pdata_start + (i as u32 * 8));
                }
                if target_addr >= split.end && split_end.is_none() {
                    split_end = Some(pdata_start + (i as u32 * 8));
                    break;
                }
            }
            ...
            match (split_start, split_end) {
                (Some(start), Some(end)) => {
                    if start != end {
                        // if start == end, there's no pdata for this split
